...
def symmetry():
    for key_pattern in plist.iteritems():
        key = key_pattern[0]
        plist[key].setSymmetryH(horiz_symmetry(4,key_pattern))
        plist[key].setSymmetryV(vert_symmetry(4,key_pattern))
    for key_pattern in glist.iteritems():
        key = key_pattern[0]
        glist[key].setSymmetryH(horiz_symmetry(4,key_pattern))
        glist[key].setSymmetryV(vert_symmetry(4,key_pattern))
    for key_pattern in rlist.iteritems():
        key = key_pattern[0]
        rlist[key].setSymmetryH(horiz_symmetry(4,key_pattern))
        rlist[key].setSymmetryV(vert_symmetry(4,key_pattern))

def stroke():
    for key_pattern in plist.iteritems():
        key = key_pattern[0]
        plist[key].setStroke(strokeLen(4,key_pattern))
    for key_pattern in glist.iteritems():
        key = key_pattern[0]
        glist[key].setStroke(strokeLen(4,key_pattern))
    for key_pattern in rlist.iteritems():
        key = key_pattern[0]
        rlist[key].setStroke(strokeLen(4,key_pattern))
...
if(__name__ == "__main__"):    
    drawGrid('0.1.2.3.6.9.12.13.14.15')
    #main(sys.argv[1:])
